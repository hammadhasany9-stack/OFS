...
const navLinks = [
  { href: '/single-site-printroom', label: 'Single Site', disabled: true },
  { href: '/dtp-printroom', label: 'Direct to Patient', disabled: false },
  { href: '/at-home-printroom', label: 'At-Home', disabled: true },
]

export function PrintRoomTopNav() {
  const pathname = usePathname()
  const router = useRouter()
  const [isDark, setIsDark] = useState(false)
  const { orders } = useOrders()
  const { user } = useUser()
  const inprocessCount = orders.filter(order => order.status === 'Inprocess').length
  ...
  const handleLogout = () => {
    clearAuthenticated('printroom')
    router.push('/dtp-printroom/login')
  }
    ...
    <nav className="sticky top-0 z-50 w-full border-b bg-white/95 backdrop-blur-sm dark:bg-zinc-950/95 dark:border-zinc-800/50 shadow-sm transition-colors duration-200">
      <div className="flex h-16 items-center justify-between px-6 max-w-[1600px] mx-auto">
        <div className="flex items-center gap-8">
          <Link href="/dtp-printroom" className="flex items-center gap-3 group">
          ...
          <div className="flex items-center gap-1 ml-4">
            {navLinks.map((link) => {
              ...
              const badgeCount = link.href === '/dtp-printroom' ? inprocessCount : 0
              ...
              return link.disabled ? (
                ...
                >
                  ...
                  <Badge
                    variant="secondary"
                    className="h-5 min-w-5 px-1.5 text-xs font-semibold bg-gray-200 text-gray-400 dark:bg-zinc-800 dark:text-zinc-600"
                  >
                    {badgeCount}
                  </Badge>
                </div>
